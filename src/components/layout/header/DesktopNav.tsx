...
export const DesktopNav: React.FC<DesktopNavProps> = ({
  ...
  return <nav className="hidden md:flex items-center space-x-8">
      <Link to="/marcas" className="font-semibold text-lg hover:text-imperio-navy transition-colors relative group">
        <span>Marcas</span>
        <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-imperio-navy group-hover:w-full transition-all duration-300"></span>
      </Link>
      <Link to="/fretes" className="font-semibold text-lg hover:text-imperio-navy transition-colors relative group">
        <span>Fretes</span>
        <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-imperio-navy group-hover:w-full transition-all duration-300"></span>
      </Link>
      <div className="relative group" onMouseEnter={() => setCategoryMenuOpen(true)} onMouseLeave={() => setCategoryMenuOpen(false)}>
